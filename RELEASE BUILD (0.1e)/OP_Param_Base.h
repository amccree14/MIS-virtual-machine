...
{
	public:
			// using c-style enum for switch-case format in the
			//	execute() functions
		enum OP_TYPE{NUMERIC, REAL, CHAR, STRING,
		...
		//~OP_Param_Base(){}
			...
			// pure virtual functions for lock and unlock, only actually
			//	usable in Variable and V_Param. Normal parameters will
			//	throw exceptions if these are somehow called.
		virtual void lock(std::thread* id) = 0;
		virtual void unlock(std::thread* id) = 0;
