...
class MIS_System
{
	...
	public:
		...
		bool parse(char* filename);			// parse file into vectors
		
				// execute through the command vector, takes an int ID
				//	of the current thread, as well as a pointer to the
				//	current thread's handle (for passing to thread_exit)
		void run(unsigned int id, std::thread **cthread, unsigned long long nPC);
		
		void init(Command_DLL*);
		void quit();
